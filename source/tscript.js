...
let core = {
	...
	"impl": {
			...
			"setEventHandler": function(event, handler) {
				...
				if (handler.type.id != module.typeid_none && ! module.isDerivedFrom(handler.type, module.typeid_function)) this.error("/argument-mismatch/am-1", ["handler", "setEventHandler", "null or function", module.displayname(handler.type)]);
				if (handler.value.b.func.params.length != 1) this.error("/argument-mismatch/am-41");
...
module.Interpreter = function(program)
{
	...
	this.service.canvas = {
		...
		"setLineWidth": function(width) {
			if (! this.service.canvas.dom) return;
